...
- legacy registration audit and reconstruction utilities for retained agent workspaces; these are not a live container dependency
...
- Bluebeam BTX intake is documented in `../docs/btx-submission-workflow.md`. Vlad converts each supported BTX symbol into SVG, DXF, and PNG; the persisted PNG derivative is the per-child review preview, while SVG/DXF remain derived/download assets. Successful libraries escalate through the existing `raster_split_review` child-review stage rather than silently passing or publishing.
