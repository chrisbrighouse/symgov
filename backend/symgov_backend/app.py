...
def create_app() -> FastAPI:
    ...
    async def http_exception_handler(_, exc: StarletteHTTPException) -> JSONResponse:
        ...
        if isinstance(exc.detail, dict):
            ...
            detail = str(message_value) if isinstance(message_value, str) and message_value else "Request failed."
        else:
            detail = exc.detail if isinstance(exc.detail, str) else "Request failed."
        error = "not_found" if exc.status_code == 404 else "request_error"
        response_body = {"error": error, "detail": detail}
        if code:
    ...
    app.include_router(
        symbol_sets_default_router,
        prefix=settings.api_prefix,
        dependencies=[Depends(stage4_route_guard), csrf, session_access],
    )
    app.include_router(
        symbol_context_router,
        prefix=settings.api_prefix,
        dependencies=[Depends(stage4_route_guard), csrf, session_access],
    )
    app.include_router(
        organization_symbols_router,
        prefix=settings.api_prefix,
        dependencies=[Depends(organization_symbols_route_guard), csrf, session_access],
    )
    app.include_router(
