...
def is_short_symbol_display_id(value: object) -> bool:
    return bool(re.fullmatch(r"[0-9A-F]{4}-\d+", str(value or "").strip(), flags=re.IGNORECASE))
...
def build_provenance_workspace_item(
    ...
    payload = {
        ...
        "rightsStatus": provenance_assessment.rights_status,
        "rightsDisposition": provenance_assessment.rights_disposition,
        "processingOutcome": provenance_assessment.processing_outcome,
        "sourceFileName": source_file_name,
        "sourceObjectKey": source_object_key,
        "sourcePreviewUrl": build_source_preview_url(
            str(review_case.id), source_preview_asset.get("object_key") if source_preview_asset else None
...
def hannah_photo_candidate_response(row) -> WorkspaceHannahPhotoCandidateResponse:
    ...
    return WorkspaceHannahPhotoCandidateResponse(
        ...
        rightsDisposition=candidate.evidence_json.get("rights_disposition") if candidate.evidence_json else None,
        processingOutcome=candidate.evidence_json.get("processing_outcome") if candidate.evidence_json else None,
        riskLevel=candidate.evidence_json.get("risk_level") if candidate.evidence_json else None,
