...
**Likely files:** `backend/symgov_backend/services/published_feedback.py`, `backend/symgov_backend/routes/published.py`, published read-model tests and Ed queue tests.

**Acceptance:** every feedback/review-request route preserves the published read model, creates the expected durable workflow, records the authenticated or API-key requester, and has negative tests proving no implicit lifecycle change. This closes before any new reviewer, capability or submission feature work.
