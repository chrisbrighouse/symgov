...
**Done (2026-09-06).** Full evidence: `docs/plans/2026-09-06-stage11-wp11.5-regression-performance-accessibility-evidence.md`. Portable regression: 2292 passed, 6 pre-existing (confirmed on unmodified `origin/main`) failures, 811s. External-workspace partition (run separately — `test-backend.sh --full`'s own `set -eu` skips it once the portable partition reports any failure): all clean. `npm run test:frontend` 250 passed, `npm run build` clean, frozen compile gate clean, secret scan clean, `git diff --check` clean. Performance: `tests/test_stage11_wp11_5_performance_evidence_postgresql.py` proves the WP11.1-widened eligibility join is index-backed (no sequential scan) at a 3,000-symbol dataset and records illustrative endpoint timings. Advisory-agent fail-closed/non-blocking proven structurally (only `routes/agents.py` depends on either agent module). Accessibility review found and fixed one real gap: the new Symbol Set Builder filter controls' wrapper `<div>` had `aria-label` with no ARIA role (a bare `div`'s implicit role does not support author naming) — added `role="group"`, matching this file's own existing convention.
...
**Done (2026-09-06).** Full evidence: `docs/plans/2026-09-06-stage11-wp11.6-independent-review.md`. **Decided:** no Hermes Kanban board was reachable this session (no MCP tool configured; the dashboard URL returned 403) — Chris explicitly approved substituting a rigorous independent manual review for both lanes and skipping the Kanban process for this stage. Release candidate hash-locked at commit `263c1cf` (diff SHA-256 `639b0b9e...`) before review. Both lanes: PASS WITH NOTES. Three findings were corrected (not merely noted): a fabricated "Clear filters" UI-copy claim in the WP11.5 evidence doc; WP11.3's API-key tests, which were renamed/re-documented after independent verification showed one was structural (no session cookie) rather than API-key-specific, and the other's original "doesn't consult CatalogApiKey" claim was itself checked and found incorrect (empirically confirmed the route does require a valid key) — both now documented accurately; and the WP11.5 performance test's index-usage assertion, which was rewritten to EXPLAIN the actual compiled statement (via a `before_cursor_execute` capture) instead of a hand-duplicated SQL string. One item (no server-side row cap on the organization-private half of Builder search) was recorded as a known, pre-existing, out-of-scope deferral rather than fixed. All corrections re-verified green (13/13 WP11.3 tests, WP11.5 performance test, full compile gate, secret scan, `git diff --check`).
...
**Done (2026-09-07).** Full evidence: `docs/plans/2026-09-06-stage11-wp11.7-migration-rehearsal.md`. Every individual migration transition already had its own dedicated, currently-passing downgrade/reupgrade rehearsal against real disposable Postgres (cited, not rebuilt: `test_project_symbol_set_postgresql.py`'s WP1 empty/populated guard tests, `test_organization_symbol_postgresql.py`'s visibility-floor clean/populated-refusal tests, `test_wp76_...`'s real-demotion floor-guard test, `test_public_projection_migration.py`'s Stage-7 downgrade + single-head proof, `test_organization_postgresql_migration.py`'s bootstrap/audit-immutability suite). The genuine gap — the whole chain in one continuous motion — is closed by `tests/test_stage11_wp11_7_migration_rehearsal_postgresql.py` (6 tests): restores to the true pre-organization revision (`20260802_0026`), seeds a legacy dataset, upgrades straight through to current head (`20260905_0044`) in one motion, confirms the legacy dataset survives exactly ownerless-public-equivalent; bootstrap dry-run/apply/idempotency; two real tenant organizations seeded on that same accumulated database with zero cross-tenant membership rows; schema-first pre-floor rollback + reupgrade on a clean database; rollback discipline re-confirmed correct (neither falsely tripped nor bypassed) on top of unrelated accumulated Stage 1-10 data; single Alembic head plus a direct referential-integrity spot-check (zero orphaned memberships/symbols). Emergency pre-floor recovery ordering is recorded as a procedural checklist (deployment-topology action, not an internal flag this repository can flip and assert against — the `published`/Catalog routes have no such flag at all). All database URLs/credentials redacted per the existing disposable-container placeholder convention.
...
**Done (2026-09-07).** Full walkthrough: `docs/plans/2026-09-06-stage11-wp11.9-acceptance-checklist.md`. **Count correction:** the checklist at programme-plan lines 1037-1060 is 24 items, not 20 as this plan's own prose (and the kickoff prompt) label it — verified by re-reading the exact range rather than trusting the stated count. All 24 items close with direct evidence (this stage's own, or cited from a prior stage's own closing audit); the icon-malware-scan and live-Hermes-binding items are reconfirmed as intentional deferrals, not closed. One real, previously unflagged gap was found and closed while writing the walkthrough: `./scripts/test-langfuse-poc.sh` had never actually been run this stage — run now, 12 passed (synthetic-contract suite, no live Langfuse dependency).
